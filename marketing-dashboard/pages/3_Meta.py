"""Meta (Facebook / Instagram) campaigns view."""
...
st.set_page_config(page_title="Meta | Marketing Dashboard", layout="wide")
st.title("Meta Ads (Facebook & Instagram)")

date_range, granularity = render_date_picker(key_prefix="meta")
...
if fetch_btn or "meta_df" not in st.session_state:
    with st.spinner("Fetching Meta data..."):
        from config.settings import settings
        results, errors = fetch_platforms(
            ["meta"], date_range, granularity, conn, ttl_seconds=settings.CACHE_TTL_SECONDS
        )

    for platform, msg in errors.items():
        st.warning(f"**Meta**: {msg}", icon="⚠️")

    df = resample_to_granularity(
        rows_to_dataframe(results.get("meta", [])), granularity
    )
    if not df.empty:
        df = calculate_derived_metrics(df)
    st.session_state.meta_df = df

df = st.session_state.get("meta_df")
if df is None or df.empty:
    st.info("Click **Fetch Data** to load Meta reports.")
    st.stop()
...
with col1:
    st.plotly_chart(spend_over_time_line(df, "meta"), use_container_width=True)
with col2:
    st.plotly_chart(conversions_over_time_line(df, "meta"), use_container_width=True)

st.divider()
render_metrics_table(df, show_platform_col=False, show_ga4_cols=False, title="Meta Campaigns")
